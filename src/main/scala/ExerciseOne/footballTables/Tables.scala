...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Country.schema, Game.schema, Goal.schema, Goalscorers.schema, Helpgoalconsistence.schema, Results.schema, Shootout.schema, Shootouts.schema, Team.schema).reduceLeft(_ ++ _)
  ...
  /** Entity class storing rows of table Helpgoalconsistence
   *  @param id Database column ID SqlType(INTEGER)
   *  @param teamId Database column TEAM_ID SqlType(INTEGER)
   *  @param gameId Database column GAME_ID SqlType(INTEGER)
   *  @param awayTeamScored Database column AWAY_TEAM_SCORED SqlType(BOOLEAN)
   *  @param awayTeam Database column AWAY_TEAM SqlType(INTEGER)
   *  @param homeTeam Database column HOME_TEAM SqlType(INTEGER)
   *  @param homeScore Database column HOME_SCORE SqlType(INTEGER)
   *  @param awayScore Database column AWAY_SCORE SqlType(INTEGER) */
  case class HelpgoalconsistenceRow(id: Option[Int], teamId: Option[Int], gameId: Option[Int], awayTeamScored: Option[Boolean], awayTeam: Option[Int], homeTeam: Option[Int], homeScore: Option[Int], awayScore: Option[Int])
  /** GetResult implicit for fetching HelpgoalconsistenceRow objects using plain SQL queries */
  implicit def GetResultHelpgoalconsistenceRow(implicit e0: GR[Option[Int]], e1: GR[Option[Boolean]]): GR[HelpgoalconsistenceRow] = GR{
    prs => import prs._
    HelpgoalconsistenceRow.tupled((<<?[Int], <<?[Int], <<?[Int], <<?[Boolean], <<?[Int], <<?[Int], <<?[Int], <<?[Int]))
  }
  /** Table description of table HELPGOALCONSISTENCE. Objects of this class serve as prototypes for rows in queries. */
  class Helpgoalconsistence(_tableTag: Tag) extends profile.api.Table[HelpgoalconsistenceRow](_tableTag, "HELPGOALCONSISTENCE") {
    def * = (id, teamId, gameId, awayTeamScored, awayTeam, homeTeam, homeScore, awayScore).<>(HelpgoalconsistenceRow.tupled, HelpgoalconsistenceRow.unapply)
    ...
    /** Database column GAME_ID SqlType(INTEGER) */
    val gameId: Rep[Option[Int]] = column[Option[Int]]("GAME_ID")
    /** Database column AWAY_TEAM_SCORED SqlType(BOOLEAN) */
    val awayTeamScored: Rep[Option[Boolean]] = column[Option[Boolean]]("AWAY_TEAM_SCORED")
    /** Database column AWAY_TEAM SqlType(INTEGER) */
    val awayTeam: Rep[Option[Int]] = column[Option[Int]]("AWAY_TEAM")
    /** Database column HOME_TEAM SqlType(INTEGER) */
    val homeTeam: Rep[Option[Int]] = column[Option[Int]]("HOME_TEAM")
    /** Database column HOME_SCORE SqlType(INTEGER) */
    val homeScore: Rep[Option[Int]] = column[Option[Int]]("HOME_SCORE")
    /** Database column AWAY_SCORE SqlType(INTEGER) */
    val awayScore: Rep[Option[Int]] = column[Option[Int]]("AWAY_SCORE")
  }
  /** Collection-like TableQuery object for table Helpgoalconsistence */
  lazy val Helpgoalconsistence = new TableQuery(tag => new Helpgoalconsistence(tag))
